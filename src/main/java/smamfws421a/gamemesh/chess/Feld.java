...
public class Feld implements MouseListener {
    ...
    public void reset(){
        ...
        this.playingBoard[2][0].setFigur(new Laufer(black));
        this.playingBoard[5][0].setFigur(new Laufer(black));
        this.playingBoard[2][7].setFigur(new Laufer(white));
        this.playingBoard[5][7].setFigur(new Laufer(white));

        this.playingBoard[4][0].setFigur(new Konig(black));
        this.playingBoard[4][7].setFigur(new Konig(white));
        ...
        for (int i = 0; i < 8; i++){
            for (int j = 0; j < 8; j++){
                if (Feld.playingBoard[i][j].getFigur() != null) {
                    ...
                    for (int x = 0;x<8;x++){
                        for (int y = 0; y < 8; y++){
                            if (Feld.playingBoard[i][j].getFigur() instanceof Konig && Feld.playingBoard[i][j].getAttacked(farbe)[x][y]) {
