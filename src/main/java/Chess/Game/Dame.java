...
public class Dame extends Figur {
    ...
    public BufferedImage getImage() {
        if (getColor()){
            return Chess.Game.ImageLoader.FigurW[4];
        }else {
    ...
    public boolean[][] Bewegung(Chess.Game.Feld feld) {
        ...
        for(int i = -1; i <= 1; i=i+2){
            for (int j = -1; j <= 1; j += 2){
                xcur = x + i;
                ycur = y + j;
                while (xcur >= 0 && xcur <= 7 && ycur >= 0 && ycur <= 7 && !(Chess.Game.Feld.playingBoard[xcur][ycur].isOccupied())){
                    allowedFields[xcur][ycur] = true;
                    xcur = xcur + i;
                    ycur = ycur + j;
                }
                if (xcur >= 0 && xcur <= 7 && ycur >= 0 && ycur <= 7 && Chess.Game.Feld.playingBoard[xcur][ycur].getFigur().getColor() != this.color) {
                    allowedFields[xcur][ycur] = true;
                }
            }
            ycur = y + i;
