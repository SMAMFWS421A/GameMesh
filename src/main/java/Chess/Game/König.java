...
public class König extends Figur {
    ...
    public BufferedImage getImage() {
        if (getColor()){
            return Chess.Game.ImageLoader.FigurW[5];
        }else {
                ...
                for (int a = 0; a < 8; a++){
                    for (int b = 0; b < 8;b++){
                        if (xcur >= 0 && xcur <= 7 && ycur >= 0 && ycur <= 7) {
                            ...
                            if (Chess.Game.Feld.playingBoard[xcur][ycur].getAttacked(farbe)[a][b]){
                                if (!Chess.Game.Feld.playingBoard[xcur][ycur].isOccupied()) {
                                    allowedFields[xcur][ycur] = false;
                                } else if (Chess.Game.Feld.playingBoard[xcur][ycur].getFigur().getColor() == this.color) {
                                    allowedFields[xcur][ycur] = false;
                                }
                            }
                        }
